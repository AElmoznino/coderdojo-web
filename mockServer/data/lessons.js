module.exports = [
  {
    lessonId: 'nybörjare-1',
    lessonTitle: 'HTML del 1',
    lessonShortFacts: null,
    lessonIntro: `HTML används för att strukturera upp hemsidor. Alla hemsidor är byggda med det.`,
    level: 'nybörjare',
    nextLesson: {
      lessonId: 'nybörjare-2',
      title: 'HTML del 2',
    },
    objectives: [
      'strukturera upp din hemsida med HTML',
      'inspektera andra hemsidor för att se hur de är uppbyggda',
    ],
    sections: [
      {
        sectionBody: `HTML står för Hypertext Markup Language. Det är instruktioner för webbläsaren hur sidan ska ritas upp i din webbläsare.`,
        sectionFinishingText: null,
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null,
        sectionTitle: 'Vad är HTML',
      },
      {
        sectionBody: `HTML består av olika så kallade "taggar" som är förbestämda. En rubrik skrivs som <h1>, <h2> osv beroende på storlek.`,
        sectionFinishingText: 'Här kommer mer text',
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null, // 'https://codesandbox.io/embed/kwnvw1wvl3?module=%2Fsrc%2Fmatte.js'
        sectionTitle: 'Hur skriver man HTML',
      },
    ],
  },
  {
    lessonId: 'nybörjare-2',
    lessonTitle: 'HTML del 2',
    lessonShortFacts: null,
    lessonIntro: `Dags att vässa dina HTML-kunskaper med lite övningar`,
    level: 'nybörjare',
    nextLesson: {
      lessonId: 'nybörjare-3',
      title: 'CSS del 1',
    },
    objectives: ['göra mer saker med HTML', 'bygga en egen hemsida'],
    sections: [
      {
        sectionBody: `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`,
        sectionFinishingText: null,
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null,
        sectionTitle: 'Mer om HTML',
      },
      {
        sectionBody: `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`,
        sectionFinishingText: 'Här kommer mer text',
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null, // 'https://codesandbox.io/embed/kwnvw1wvl3?module=%2Fsrc%2Fmatte.js'
        sectionTitle: 'Ännu mer om HTML',
      },
    ],
  },
  {
    lessonId: 'nybörjare-3',
    lessonTitle: 'CSS del 1',
    lessonShortFacts: null,
    lessonIntro: `Dags att börja lära dig snygga till din hemsida med CSS`,
    level: 'nybörjare',
    nextLesson: {
      lessonId: 'nybörjare-4',
      title: 'CSS del 2',
    },
    objectives: ['styling med CSS', 'ändra storlekar, färger m.m.'],
    sections: [
      {
        sectionBody: `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`,
        sectionFinishingText: null,
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null,
        sectionTitle: 'Vad är CSS',
      },
      {
        sectionBody: `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`,
        sectionFinishingText: 'Här kommer mer text om CSS',
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null, // 'https://codesandbox.io/embed/kwnvw1wvl3?module=%2Fsrc%2Fmatte.js'
        sectionTitle: 'Ännu mer om CSS',
      },
    ],
  },
  {
    lessonId: 'nybörjare-4',
    lessonTitle: 'CSS del 2',
    lessonShortFacts: null,
    lessonIntro: `Dags att vässa dina CSS-kunskaper med lite övningar`,
    level: 'nybörjare',
    nextLesson: null,
    objectives: ['bygga ett snyggt Pokédex', 'och ännu mer...'],
    sections: [
      {
        sectionBody: `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`,
        sectionFinishingText: null,
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null,
        sectionTitle: 'Mer om CSS',
      },
      {
        sectionBody: `Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`,
        sectionFinishingText: 'Här kommer mer text om CSS',
        sectionImgAlt: null,
        sectionImgUrl: null,
        sectionSandbox: null, // 'https://codesandbox.io/embed/kwnvw1wvl3?module=%2Fsrc%2Fmatte.js'
        sectionTitle: 'Ännu mer om CSS',
      },
    ],
  },
]
